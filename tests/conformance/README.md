...
| `create_disk_prediction_sensor\|path\|calibration_requests\|post_period_ms\|refresh_period_ms\|free_space_start\|drain_per_second` | (#1426) the "Free space on disk prediction" sensor over a SCRIPTED free-space series (free space falls linearly with elapsed time — a function of the clock, not of the call count, so both drivers see the same drain), with fixture-sized periods; the catalog row runs 5 min posts / 30 s sampling / 6 calibration requests. The source samples free space every `refresh_period_ms` to feed the drain-speed EMA and posts a TimeSpan every `post_period_ms`. C# registers `FreeDiskSpacePredictionBase` over a scripted `IDiskInfo`; native binds a metric source running the same `disk_prediction.hpp` math. Wall-clock: the NUMBER of calibration posts is not contractual (managed's `StartAsync` resets the request counter after `InitAsync` already scheduled the first post), so assert the opening post's shape and the LAST payload, not a fixed count |
