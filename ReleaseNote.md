...
* The management API is now self-describing: full OpenAPI coverage at `/api/swagger` (bearer security scheme, per-operation response schemas with documented error codes, enum value tables on every byte field), and every error response on `/api` carries the uniform JSON body `{error, message, details}` — never HTML.
