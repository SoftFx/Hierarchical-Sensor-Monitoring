...
`Kestrel:TrustedProxies` is a list of reverse-proxy addresses (IP, CIDR such as `172.18.0.0/16`, or `attached-networks` for the networks of the server's own interfaces) whose `X-Forwarded-For` header HSM trusts for the client address. It is empty by default: HSM then records the address of whoever connects to it. The Docker Compose setup fills it in for its own Caddy; you do not need to set it yourself. It is read **from environment variables only** (`Kestrel__TrustedProxies__0=...`, `Kestrel__TrustedProxies__1=...`): a value in `appsettings.json` is ignored, and the server logs a warning. An empty value switches the trust off.
