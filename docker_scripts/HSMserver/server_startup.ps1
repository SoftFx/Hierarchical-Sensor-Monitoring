...
if ($ExpectedImageId)
{
	...
	docker run -d -it --name "$ContainerName_$Version" -v $LogsFolder -v $SensorConfigFolder -v $EnvironmentDatabaseFolder -p $SensorDataPort -p $SensorSitePort $ExpectedImageId
}
else
{
