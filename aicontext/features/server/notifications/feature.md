...
- `src/tests/HSMServer.Core.Tests/Notifications/ChatSensorUsageCalculatorTests.cs` — #1310 coverage of the per-sensor effective-chat-set seam (`ChatSensorUsageCalculator.GetEffectiveChats`): dedup when two regular policies target the same chat, regular+TTL union, folder-`DefaultChats` union and overlap dedup, folder-only case, null/empty safe skipping, and the `ChatViewModel.SensorUsageBadgeText` singular/plural rule (`0 sensors` / `1 sensor` / `N sensors`).
