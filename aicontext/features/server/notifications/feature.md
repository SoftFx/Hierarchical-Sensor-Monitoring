...
- `src/tests/HSMServer.Core.Tests/Notifications/ChatFolderBindingTests.cs` — consolidated post-#1261 coverage of `IChatsManager` folder binding. Pins: (a) `RemoveFolderFromChats_DoesNotDeleteChatAtZeroFolders` — a chat transitions to global and stays in the manager; (b) `GetAvailableChats_IncludesZeroFolderChat` / `GetAvailableChats_BoundChatExcludedFromOtherFolder` — global-default picker behavior and narrowing; (c) `AddFolderToChats_AddsFolderIdToChatFolders` / `AddFolderToChats_SingleInvocation_AddsFolderOnce` — single-event fan-out post-merge (regression for the #1261 manager merge that collapsed the Telegram + Slack subscriber pair into one); (d) `RemoveChatHandler_PrunesIdFromAllFolders` — the merged handler prunes the removed chat id from every folder.Chats reference (regression for the dual→single handler merge); (e) `Chat_WithSlackWebhookOnly_HasNoTelegramChatId` / `Chat_WithTelegramOnly_HasNoSlackWebhook` / `Chat_WithBothChannels_KeepsTelegramAndSlackFields` — pin the optional-channel invariants that let each sender null-guard its own field; (f) `Chat_MultiChannel_AccumulatorsAreIndependent` — regression for the shared-buffer bug fixed in PR #1265 (pre-fix, draining Telegram bumped the shared timer and starved Slack; per-channel `ChannelAccumulator` isolates state). Extended in #1288 to cover three channels (Telegram + Slack + Mattermost) — drains Telegram, then Slack, and asserts the third accumulator's next-send timer is still unchanged. Extended in #1304 with four `TryConnect` cases: `TryConnect_WithChatIdToken_BindsTelegramToExistingChat` (existing chat is bound in place, no orphan created, `TelegramChatId`/`TelegramType`/`AuthorizationTime` populated), `TryConnect_WithFolderToken_StillCreatesNewChat` (regression guard for the legacy folder-scoped create-new path), `TryConnect_ChatIdToken_TargetAlreadyBound_ReturnsFailed` (strict refuse on rebind — target's existing `TelegramChatId` is preserved), `TryConnect_ChatIdToken_TelegramChatOwnedByAnotherRecord_ReturnsFailedAlreadyBound` (strict refuse on theft — incoming Telegram chat already in `_telegramChatIds` under another Chat.Id; owner name surfaced in `result.Name`). Plus two pre-allocated-guid cases: `TryConnect_ChatIdToken_PreAllocatedGuid_CreatesNewGlobalChat` (chatId in token does not resolve → `CreateChatWithTelegram` materialises a new global Chat carrying the pre-allocated guid, Telegram binding populated, `Folders` empty), `TryConnect_PreAllocatedGuid_TelegramChatOwnedByAnother_ReturnsFailedAlreadyBound` (theft guard still applies to the pre-allocation path — incoming Telegram chat already in `_telegramChatIds` → `FailedAlreadyBound`, pre-allocated guid not materialised). Plus self-heal: `TryConnect_ChatIdToken_StaleIndexEntry_SelfHealsAndBinds` (stale `_telegramChatIds` entry whose owner.TelegramChatId is null → entry dropped, bind proceeds, owner untouched).
...
- `src/tests/HSMServer.Core.Tests/Notifications/ChatSensorUsageCalculatorTests.cs` — #1310 coverage of the per-sensor effective-chat-set seam (`ChatSensorUsageCalculator.GetEffectiveChats`): dedup when two regular policies target the same chat, regular+TTL union, folder-`DefaultChats` union and overlap dedup, the gating rule (folder-default chats are folded in ONLY when `includeFolderChats` is true — mirroring the alert-capable-policy check that `Compute()` makes before passing them in), null/empty safe skipping, and the `ChatViewModel.SensorUsageBadgeText` singular/plural rule (`0 sensors` / `1 sensor` / `N sensors`). Also pins the incomplete-count path: `SensorUsageBadgeText_Incomplete_PrependsGreaterThanOrEqual` (the `≥` prefix under `SensorUsageIncomplete = true`, with `1,247` group-separator formatting under invariant culture) and the matching title caveats.
- `src/tests/HSMServer.Core.Tests/Notifications/ChatSensorUsageCalculatorComputeTests.cs` — `Compute()` entry-point coverage (empty cache → empty counts, `skipped == 0`) plus the calculator's parent-chain pins: `Compute_FromParentUnderNonInheritingMiddle_StopsAtMiddle` (chain `root → mid(Custom, not FromParent) → leaf(FromParent)` resolves only mid's chats, not root's — the parent-chain rule shared with delivery since #1330) and `Compute_AllAncestorsFromParent_WalksToRoot` (chain with every ancestor in FromParent mode walks all the way up). `Compute_FromParentPolicyWithExtraChats_CountsBothInheritedAndExplicit` pins that a FromParent policy with additional `Destination.Chats` counts both the inherited parent-chain chats AND the explicit extras — a first-class state (alert form JS preserves chats in FromParent mode, alert import does the same, `PolicyDestination.ToString` has a dedicated "from parent chats, {extra}" case) that the calculator used to drop, silently undercounting.
- `src/tests/HSMServer.Core.Tests/Model/Policies/PolicyGetParentChatsTests.cs` — #1330 coverage of the delivery-side resolver `Policy.GetParentChats` (and the `TargetChats` delivery entry point): a FromParent descendant under a non-inheriting middle node (Custom or Empty mode) resolves only up to that node — the three descendant/`TargetChats` tests fail against the pre-#1330 implementation, which walked every strict ancestor; all-FromParent chains walk to root; two-level chains include both chats (regression guard against over-eager break); null parent returns empty.
...
- `POST EditChat(ChatViewModel)` — updates any updatable field on an existing chat (Slack/Mattermost webhooks, name, description, enable flag, aggregation delay). Telegram-bound fields are init-only. **Webhook masking (#1329):** webhooks are masked on read — `ChatViewModel(Chat, …)` applies `WebhookUrlMasker.Mask` (scheme+host without userinfo, all path segments except the last verbatim, last segment reduced to head4 + `••••` + tail4; entirely masked when too short to reveal safely). The mask doubles as the POST sentinel: posting it back unchanged or posting an empty field means "no change" (`ResolveWebhook` → null → `Chat.ApplyUpdate`'s `?? current`); a partially-edited mask is rejected with an inline error (exact-match against a freshly recomputed `Mask(stored)`), and clearing a stored webhook via an empty field is rejected with a pointer to the per-channel Remove button — the only clear path. URL-shape validation (absolute http/https) lives in `NotificationsController.ValidateWebhooks`, not in a DataAnnotation (`[Url]`/`type="url"` were removed with masking); a rejection re-render restores server-owned state (folders incl. hidden inputs and pending selections, masked webhook values, Telegram title/description) and activates the failing channel's tab. Unit coverage: `WebhookUrlMaskerTests`.
...
- `Views/Configuration/_Chats.cshtml` is the Chats list. After the #1281 rebuild it follows the Members-layout pattern (`Views/Account/Users.cshtml`): a search-by-name input + a channel-filter `<select>` (All channels / Telegram / Slack / Mattermost; single-select — pick one channel to filter by, or leave on All channels to show all) above a `.chat-list` of rows. Each row shows brand icons + chat Name + an Enabled/Disabled badge, with inline Edit (`fa-pen-to-square`, links to `EditChat`) and Remove (`fa-trash-can`, opens `_ConfirmationModal` then POSTs to `RemoveChat`) buttons. Each row also carries a folder-bindings badge cluster between the Name/Enabled badge and the action buttons — up to three `.chat-folder-badge` anchors (click → `FoldersController.EditFolder`) plus a `+N more` popover link when the chat is bound to more than three folders, with a blue `Public` pill for chats with zero bindings (a zero-binding chat is global — appears in every folder's destination picker, see "Folder binding" below; mirrors the Users products-badge pattern, backed by `ChatFoldersViewModel.DisplayFolders` populated in `ChatsSettingsViewModel(IChatsManager, IFolderManager)` post-#1284). Author and Created are no longer shown. Send-test-message actions were removed from the list — they already live on the `EditChat` form (`EditChat.cshtml`) per channel, so Edit is the single entry point for test sends. "Add new chat" links to `AddChat` (admin-only).

  Post-#1310 each row also carries a `.chat-badge.badge-usage` pill (e.g. `0 sensors`, `1 sensor`, `5 sensors`, `1,247 sensors`) immediately after the Enabled/Disabled badge. It counts the **unique sensors whose effective alert configuration references this chat** — a sensor that references the same chat through multiple policies (or through both a regular policy and a TTL policy) still counts once. The count is computed server-side per `Index` request by `ChatSensorUsageCalculator` (`src/server/HSMServer/Notifications/Chats/ChatSensorUsageCalculator.cs`), which iterates `ITreeValuesCache.GetSensors()` and, for each sensor, resolves each policy's effective destination once.

  What is counted per sensor:
    - Each policy's resolved chat set — for `Custom` / `Empty` / `All` modes, `Destination.Chats.Keys` directly (no per-policy HashSet allocation); for `FromParent`, the memoized ancestor-chain set with `Destination.Chats` layered on top (TryAdd semantics — explicit chats do not overwrite inherited ones). FromParent + extra chats is a first-class state (alert form JS keeps the chats array when switching to FromParent, alert import preserves chats only in FromParent mode, `PolicyDestination.ToString` has a dedicated "from parent chats, {extra}" case), so dropping the extras would undercount.
    - The sensor's root-folder `DefaultChats.SelectedChats` (the same folder-chats injection that `TreeValuesCache.SendAlertMessage` applies to every alert at delivery time) — folded in **only when the sensor has at least one alert-capable policy** — mirroring `AlertResult.IsValidAlert` (`Template is not null && (Destination.IsFromParentChats || effectiveIsAllChats || resolvedChats.Count > 0)`). `effectiveIsAllChats` is the AND of the parent's `DefaultChats.IsAllChats` and `Destination.IsAllChats`, NOT `Destination.IsAllChats` alone. Fresh sensors have zero policies (the default-policy seeding at `TreeValuesCache.AddSensor` is commented out), so the gating prevents every folder-default chat from being counted for every idle sensor and swamping the badge. Orphan sensors (`Parent == null`) skip the folder lookup entirely because `sensor.Root` casts to `ProductModel` unconditionally inside the getter (`BaseNodeModel.Root`) and would throw.

  What is NOT counted: the badge reflects **configuration references**, not active delivery. A disabled policy still counts (the chat is still wired into alert config and the admin should see the blast radius of editing or removing it); blocked / offtime sensors also count (`BaseSensorModel.CanSendNotifications` is not consulted), which is consistent with the configuration-reference framing but means the badge slightly overstates delivery fidelity for blocked sensors. `AllChats`-mode policies need no special handling: delivery relies on folder-chats injection (the `AllChats` flag is never expanded to concrete ids), so the folder-chats union already covers them once a policy is alert-capable.

  **Parent-chain rule (aligned with delivery):** the calculator's parent-chain resolver (`ResolveInheritedChats`) stops at the first ancestor in any mode other than `FromParent` (`Custom`/`Empty`/`NotInitialized` — the destination picker offers all of them as explicit operator choices, `Views/Shared/_DefaultChat.cshtml`) — the same rule as the destination-picker UI (`DefaultChatViewModel.GetParentChats`) and, since #1330, alert delivery (`Policy.GetParentChats` in HSMServer.Core). On `root(Custom, {chatA}) → mid(Custom, {chatB}, not FromParent) → leaf(FromParent)`, a sensor under `leaf` with a FromParent policy delivers to and counts only `chatB`. Before #1330 delivery walked every strict ancestor unconditionally (so the badge undercounted relative to real delivery); the fix is a recipient-shrink-only behavior change on chains deeper than two levels with an explicitly non-inheriting intermediate node.

  Caching: compute-on-request, no cache — admin `Index` is low-traffic and the count stays correct after any destination / folder / policy change without invalidation hooks.

  If `Compute()` skips at least one sensor (the cache mutates concurrently with reads and `PolicyDestination.Update` clears `Chats` without a lock — a per-sensor `try/catch (InvalidOperationException)` skips and increments a `skipped` counter; only the first skip is logged with the full exception, the rest roll up into one aggregate summary line), every chat's pill renders as `≥N sensors` (the `≥` prefix) with tooltip "Count may be incomplete — at least one sensor was skipped during the scan." A silent partial count on a badge whose sole purpose is judging edit/remove blast radius is worse than no count, so the uncertainty is surfaced rather than logged away. The `ChatViewModel.SensorUsageBadgeText` carries the singular/plural rule and the `≥` prefix; `SensorUsageBadgeTitle` carries the tooltip.

  Parent-chain resolution inside `ChatSensorUsageCalculator.ResolveInheritedChats` is a single linear walk up the chain, stopping at the first ancestor whose `DefaultChats.CurValue` is in any mode other than `FromParent` (`Custom`/`Empty`/`NotInitialized`). Matches the destination-picker UI and, since #1330, delivery (`Policy.GetParentChats`, pinned by `HSMServer.Core.Tests/Model/Policies/PolicyGetParentChatsTests`). The walk is memoized once per parent product for the duration of the `Compute()` pass. Pinned by `ChatSensorUsageCalculatorComputeTests.Compute_FromParentUnderNonInheritingMiddle_StopsAtMiddle` and `..._AllAncestorsFromParent_WalksToRoot`.
- `Views/Notifications/EditChat.cshtml` is the unified multi-channel edit form (Common section: Name, Description, EnableMessages, MessagesDelay; tabs for Telegram / Slack / Mattermost — first configured channel wins, see issue #1271 / PR #1272). Telegram tab: bound chat id, type, authorization time — read-only for bot-connected chats, with invitation-link helpers for brand-new chats. For an unbound chat the tab first shows a "Telegram bot" info row (#1311): when the bot is configured (`BotName` + `BotToken` both set, mirroring `TelegramConfig.IsValid`), the configured `BotName` is visible without opening the setup modal (the token is never rendered); when it is not, a warning links to `Configuration?tab=telegram` — the Configuration page deep-link handler whitelists the tab id and activates it via `bootstrap.Tab`. `Show setup help` behavior is unchanged in both states. Slack tab: webhook URL input with a "Show setup help" link that opens the `_SlackHelpModal.cshtml` modal — a 17-step incoming-webhook setup guide (copy carried over from #1256, retargeted to the unified form in #1275). Mattermost tab: webhook URL input with a parallel "Show setup help" link opening `_MattermostHelpModal.cshtml` — a 12-step Mattermost incoming-webhook guide (carried over from #1275; the input was disabled pre-#1288 with a "delivery not yet implemented" caveat — both removed once `MattermostNotificationChannel` shipped). Per-channel Send-test and Remove buttons live inside each tab pane and are shown only for channels the chat actually carries. Whole-chat deletion is **not** exposed from this form — the header used to carry a `Remove chat` link but it was dropped in #1275; per-channel clear (Telegram binding / Slack webhook / Mattermost webhook) stays inside the form. Whole-chat delete lives on the Chats list as an inline Remove button (`Configuration/_Chats.cshtml`, modal-confirmed — see #1281).
...
Icon markup carried in a chat option's `data-content` goes through bootstrap-select's sanitizer (`bootstrap-select.js`, `DefaultWhitelist`): anything not on the tag+attribute whitelist — e.g. an inline `svg`/`path` icon — is silently deleted at render time, with no error anywhere. Chat brand icons therefore stick to whitelisted shapes only: `<i class='fab fa-…'>` for Telegram/Slack (upgraded to SVG by the Font Awesome runtime) and a `<span class='mattermost-brand-icon' role='img' aria-label='Mattermost'>` for Mattermost — only the globally whitelisted class/role/aria attributes (`role`/`aria-label` keep the accessible name the old inline SVG carried via `<title>`; at several render sites the brand icon is the only channel indicator) — whose mask styling lives in `site.css` (`ChatIcons`; the markup shape is pinned by `ChatIconsTests`). Any future change to `ChatIcons`/`ChatBrandIconsAndName` output must keep this constraint.
...
- `OpenChatInvitationLink(Guid chatId)` / `GetChatGroupInvitation(Guid chatId)` — `[TelegramRoleFilterById(nameof(chatId), ProductRoleEnum.ProductManager)]`. The filter's base class `TelegramRoleFilterBase.TryCheckRole` has an admin-only fallback when the chat is not in storage: `AddChat()` GET pre-allocates a guid and the chat row doesn't exist yet, so the folder-loop would find zero folder ids and reject — admins are allowed through, non-admins are not (only admins can pre-allocate chats). Once the chat is created (`/start` completes), subsequent requests go through the normal folder-role check.
...
### Telegram invitation tokens

The Telegram bot binds the unified `Chat` entity to a real Telegram chat via a one-shot `/start <token>` exchange. `TokensManager.BuildInvitationToken` encodes an `InvitationToken(folderId, user)` / `InvitationToken(chatId, user)` as a guid and stores it in-memory with an expiration; `TelegramBot` `StartBot` command consumes the token via `TokensManager.TryRemoveToken` (one-shot — a second `/start` with the same token is rejected).

Two invitation modes (post-#1304):

1. **Folder-scoped** — `Views/Folders/_Chats.cshtml` renders `_NewChatHelpModal.cshtml` with `NewChatHelpViewModel { FolderId = folderId }`. The invitation links hit `NotificationsController.OpenInvitationLink(folderId)` / `GetGroupInvitation(folderId)` → `IChatsManager.GetInvitationLink(folderId, user)`. The token carries a `FolderId`. On `/start`, `ChatsManager.TryConnect` creates a brand-new `Chat`, invokes `ConnectChatToFolder(chat.Id, folderId, userName)` to bind it to the folder, and returns `ChatConnectOutcome.FolderAdded`. This is the original flow — chat does not exist before the bot `/start`.
2. **Existing-chat bind** — `Views/Notifications/EditChat.cshtml` renders the same modal with `NewChatHelpViewModel { ChatId = chat.Id }`. The links hit `NotificationsController.OpenChatInvitationLink(chatId)` / `GetChatGroupInvitation(chatId)` → `IChatsManager.GetChatInvitationLink(chatId, user)`, both gated by `[TelegramRoleFilterById(nameof(chatId), ProductRoleEnum.ProductManager)]`. The token carries a `ChatId`. On `/start`, `TryConnect` takes the early branch: `token.ChatId is Guid id && TryGetValue(id, out var existing)` → `BindTelegramToChat(existing, message)`. No new chat is created — `TelegramChatId`, `TelegramType`, `AuthorizationTime` are populated on the existing record via `TryUpdate`.
3. **Pre-allocated guid (new chat)** — `NotificationsController.AddChat()` (GET) hands the `ChatViewModel` a fresh `Guid.NewGuid()` so the EditChat form opens with a real chat guid before any row is written. The user can click "Show setup help" → invitation link → `/start` against that guid. When `/start` arrives, the chat is still not in storage, so `TryConnect`'s early branch falls through `TryGetValue` and dispatches to `CreateChatWithTelegram(chatId, message, token.User)`: materialises a new global Chat with the pre-allocated guid, populates `TelegramChatId` / `TelegramType` / `AuthorizationTime`, and leaves `Folders` empty (no folder binding — admin can bind later via EditChat or a folder's Chats tab). If the user abandons the form without `/start`, no row is ever written — no orphan.

The refactoring is necessary because pre-#1304 `EditChat` reused the folder-scoped payload with the chat guid packed into `asp-route-folderId`. The folder lookup failed silently (the guid is a chat, not a folder), `ConnectChatToFolder` returned null, and `TryConnect` dropped the chat — leaving an orphan Telegram chat that received notifications but never showed `TelegramChatId` in the admin form.

**Strict conflict policy** (in `BindTelegramToChat` and `CreateChatWithTelegram`): refuse if (a) the target Chat already has a `TelegramChatId` different from the incoming message's chat (rebind — only relevant in `BindTelegramToChat`), or (b) the incoming Telegram chat is already bound to another Chat record that still claims it (theft — checked in both helpers via `_telegramChatIds`). (a) returns `ChatConnectOutcome.Failed`; (b) returns `ChatConnectOutcome.FailedAlreadyBound` with the owner chat's name in `ChatConnectResult.Name` so the bot reply can name the record the user needs to remove. No mutation in either case. Admin must explicitly clear the Telegram binding via `EditChat` → Telegram tab → Remove (or remove the owner chat entirely) before rebinding. Without this guard, `/start` from a different Telegram chat would silently steal the binding or overwrite the target's existing binding.

**Self-heal for stale `_telegramChatIds`**: the theft guard checks `owner.TelegramChatId` after pulling the owner from the index. If the owner no longer claims the incoming Telegram chat (`owner.TelegramChatId is null` or points to a different chat), the index entry is dangling — typically from a binding cleared through a path that bypassed `TryUpdate` (dev runs, hand-edited storage, future refactor bugs). The guard drops the stale entry and proceeds with the bind instead of refusing. Only an active conflict (owner's `TelegramChatId` still matches the incoming chat) refuses. This keeps a stale index from blocking legitimate binds.

`TelegramType` and `AuthorizationTime` are `internal set` on `Chat` and not exposed by `ChatUpdate`/`ApplyUpdate`. `BindTelegramToChat` sets them directly on the model before calling `TryUpdate` — `ToEntity()` reads live model state, so the persist carries the new values. `TelegramChatTitle` is populated via the subsequent `ChatNamesSynchronization` pass (see "Telegram title/description sync" below); `TelegramChatDescription` is not set here because the inbound `Telegram.Bot.Types.Message.Chat` is the base `Chat`, not `ChatFullInfo`, and has no description field.

Bot reply (`TelegramUpdateHandler.StartBot`) switches on `ChatConnectResult.Outcome`:
- `FolderAdded` → `"Folder '{Name}' is successfully added to {direct|group by {user}}."` (existing copy).
- `ChatBound` → `"Chat '{Name}' is successfully connected to {direct|group by {user}}."`
- `FailedAlreadyBound` → `"This Telegram chat is already bound to chat '{Name}'. Remove that chat first."` — `Name` carries the owner chat's name so the user knows which record to clean up. The typical trigger is the pre-allocated-guid orphan: AddChat → `/start` writes a row the user never saved, then they try to bind the same Telegram chat to another chat. The owner chat is that orphan; the user finds it in `/Notifications/Index` (no folder bindings, name is the Telegram username/title) and removes it.
- `Failed` → `"Sorry, your token is invalid, chat doesn't exist, or this Telegram chat is already bound to another record."`
