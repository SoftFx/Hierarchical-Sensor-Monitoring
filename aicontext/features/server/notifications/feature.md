...
**Strict conflict policy** (in `BindTelegramToChat` and `CreateChatWithTelegram`): refuse if (a) the target Chat already has a `TelegramChatId` different from the incoming message's chat (rebind — only relevant in `BindTelegramToChat`), or (b) the incoming Telegram chat is already bound to another Chat record (theft — checked in both helpers via `_telegramChatIds`). (a) returns `ChatConnectOutcome.Failed`; (b) returns `ChatConnectOutcome.FailedAlreadyBound` with the owner chat's name in `ChatConnectResult.Name` so the bot reply can name the record the user needs to remove. No mutation in either case. Admin must explicitly clear the Telegram binding via `EditChat` → Telegram tab → Remove (or remove the owner chat entirely) before rebinding. Without this guard, `/start` from a different Telegram chat would silently steal the binding or overwrite the target's existing binding.
