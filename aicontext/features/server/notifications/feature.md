...
- `src/tests/HSMServer.Core.Tests/Notifications/ChatSensorUsageCalculatorComputeTests.cs` — `Compute()` entry-point coverage: empty sensor cache yields an empty counts dictionary. The per-sensor try/catch (cache mutates concurrently with reads) is exercised in production by the live `ITreeValuesCache`; `BaseSensorModel` is abstract with members Moq cannot proxy, so per-sensor counting rules live in the static-seam unit tests instead of a synthetic throw-from-getter case here.
...
- `Views/Configuration/_Chats.cshtml` is the Chats list. After the #1281 rebuild it follows the Members-layout pattern (`Views/Account/Users.cshtml`): a search-by-name input + a channel-filter `<select>` (Any channel / Telegram / Slack / Mattermost; single-select — pick one channel to filter by, or leave on Any to show all) above a `.chat-list` of rows. Each row shows brand icons + chat Name + an Enabled/Disabled badge, with inline Edit (`fa-pen-to-square`, links to `EditChat`) and Remove (`fa-trash-can`, opens `_ConfirmationModal` then POSTs to `RemoveChat`) buttons. Each row also carries a folder-bindings badge cluster between the Name/Enabled badge and the action buttons — up to three `.chat-folder-badge` anchors (click → `FoldersController.EditFolder`) plus a `+N more` popover link when the chat is bound to more than three folders, with a blue `Public` pill for chats with zero bindings (a zero-binding chat is global — appears in every folder's destination picker, see "Folder binding" below; mirrors the Users products-badge pattern, backed by `ChatFoldersViewModel.DisplayFolders` populated in `ChatsSettingsViewModel(IChatsManager, IFolderManager)` post-#1284). Author and Created are no longer shown. Send-test-message actions were removed from the list — they already live on the `EditChat` form (`EditChat.cshtml`) per channel, so Edit is the single entry point for test sends. "Add new chat" links to `AddChat` (admin-only).
  ...
  Post-#1310 each row also carries a `.chat-badge.badge-usage` pill (e.g. `0 sensors`, `1 sensor`, `5 sensors`) immediately after the Enabled/Disabled badge. It counts the **unique sensors whose effective alert destination includes this chat** — a sensor that references the same chat through multiple policies (or through both a regular policy and a TTL policy) still counts once. The count is computed server-side per `Index` request by `ChatSensorUsageCalculator` (`src/server/HSMServer/Notifications/Chats/ChatSensorUsageCalculator.cs`), which iterates `ITreeValuesCache.GetSensors()` and, for each sensor, unions `policy.TargetChats.Chats.Keys` across regular + TTL policies. The sensor's root-folder `DefaultChats.SelectedChats` (the same folder-chats injection that `TreeValuesCache.SendAlertMessage` applies to every alert at delivery time) is folded in **only when the sensor has at least one alert-capable policy** — mirroring `AlertResult.IsValidAlert` (`Template is not null && (Destination.IsFromParentChats || Destination.IsAllChats || Destination.Chats.Count > 0)`). Fresh sensors have zero policies (the default-policy seeding at `TreeValuesCache.cs:2439` is commented out), so the gating prevents every folder-default chat from being counted for every idle sensor and swamping the badge. Orphan sensors (`Parent == null`) skip the folder lookup entirely because `sensor.Root` casts to `ProductModel` unconditionally inside the getter (`BaseNodeModel.cs:50`) and would throw. Counts reflect **configuration references**, not active delivery — a disabled policy still counts, because the chat is still wired into alert config and the admin should see the blast radius of editing or removing it. `AllChats`-mode policies need no special handling: delivery relies on folder-chats injection (the `AllChats` flag is never expanded to concrete ids), so the folder-chats union already covers them. Compute-on-request, no cache — admin `Index` is low-traffic and the count stays correct after any destination / folder / policy change without invalidation hooks. The singular/plural wording lives on `ChatViewModel.SensorUsageBadgeText`.

  Parent-chain resolution inside `Policy.GetParentChats` (HSMServer.Core) is a single linear walk that stops at the first ancestor whose `DefaultChats.CurValue.IsFromParent` is false. Previously the helper reentered itself once per ancestor, producing O(2^depth) chat dictionaries on deep product trees; the union semantics (TryAdd) are unchanged, only the allocation pattern is linear now.
