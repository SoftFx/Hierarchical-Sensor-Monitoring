...
- `src/tests/HSMServer.Core.Tests/Notifications/ChatSensorUsageCalculatorTests.cs` — #1310 coverage of the per-sensor effective-chat-set seam (`ChatSensorUsageCalculator.GetEffectiveChats`): dedup when two regular policies target the same chat, regular+TTL union, folder-`DefaultChats` union and overlap dedup, the gating rule (folder-default chats are folded in ONLY when `includeFolderChats` is true — mirroring the alert-capable-policy check that `Compute()` makes before passing them in), null/empty safe skipping, and the `ChatViewModel.SensorUsageBadgeText` singular/plural rule (`0 sensors` / `1 sensor` / `N sensors`).
- `src/tests/HSMServer.Core.Tests/Notifications/ChatSensorUsageCalculatorComputeTests.cs` — `Compute()` entry-point coverage (empty cache → empty counts, `skipped == 0`) plus the GetParentChats routing pins: `GetParentChats_StopsAtFirstNonInheritingAncestor` (chain `root → mid(Custom, not FromParent) → leaf(FromParent)` stops at mid and does not include root's chats) and `GetParentChats_AllAncestorsFromParent_WalksToRoot` (chain with every ancestor in FromParent mode walks all the way up). These pin the PR #1327 routing change to `Policy.GetParentChats` that makes Core's resolution agree with `DefaultChatViewModel.GetParentChats`. `Compute_FromParentPolicyWithExtraChats_CountsBothInheritedAndExplicit` pins that a FromParent policy with additional `Destination.Chats` counts both the inherited parent-chain chats AND the explicit extras — a first-class state (alert form JS preserves chats in FromParent mode, alert import does the same, `PolicyDestination.ToString` has a dedicated "from parent chats, {extra}" case) that the calculator used to drop, silently undercounting.
...
- `Views/Configuration/_Chats.cshtml` is the Chats list. After the #1281 rebuild it follows the Members-layout pattern (`Views/Account/Users.cshtml`): a search-by-name input + a channel-filter `<select>` (Any channel / Telegram / Slack / Mattermost; single-select — pick one channel to filter by, or leave on Any to show all) above a `.chat-list` of rows. Each row shows brand icons + chat Name + an Enabled/Disabled badge, with inline Edit (`fa-pen-to-square`, links to `EditChat`) and Remove (`fa-trash-can`, opens `_ConfirmationModal` then POSTs to `RemoveChat`) buttons. Each row also carries a folder-bindings badge cluster between the Name/Enabled badge and the action buttons — up to three `.chat-folder-badge` anchors (click → `FoldersController.EditFolder`) plus a `+N more` popover link when the chat is bound to more than three folders, with a blue `Public` pill for chats with zero bindings (a zero-binding chat is global — appears in every folder's destination picker, see "Folder binding" below; mirrors the Users products-badge pattern, backed by `ChatFoldersViewModel.DisplayFolders` populated in `ChatsSettingsViewModel(IChatsManager, IFolderManager)` post-#1284). Author and Created are no longer shown. Send-test-message actions were removed from the list — they already live on the `EditChat` form (`EditChat.cshtml`) per channel, so Edit is the single entry point for test sends. "Add new chat" links to `AddChat` (admin-only).
  ...
  Post-#1310 each row also carries a `.chat-badge.badge-usage` pill (e.g. `0 sensors`, `1 sensor`, `5 sensors`, `1,247 sensors`) immediately after the Enabled/Disabled badge. It counts the **unique sensors whose effective alert destination would be delivered to this chat** — a sensor that references the same chat through multiple policies (or through both a regular policy and a TTL policy) still counts once. The count is computed server-side per `Index` request by `ChatSensorUsageCalculator` (`src/server/HSMServer/Notifications/Chats/ChatSensorUsageCalculator.cs`), which iterates `ITreeValuesCache.GetSensors()` and, for each sensor, resolves each policy's effective destination once. Resolution mirrors `Policy.TargetChats` exactly: when `Destination.IsFromParentChats`, the parent-chain chat set is resolved first and `Destination.Chats` is layered on top (TryAdd semantics — explicit chats do not overwrite inherited ones); for Custom/Empty/All modes, `Destination.Chats` is read directly. FromParent + extra chats is a first-class, reachable state — the alert form JS keeps the chats array when switching to FromParent (`_AlertsFormCollection.cshtml`), alert import preserves chats only in FromParent mode (`AlertExportViewModel.cs`), and `PolicyDestination.ToString` has a dedicated "from parent chats, {extra}" case — so dropping them was an undercount in exactly the failure direction that matters for a blast-radius badge. The inherited parent-chain set is memoized once per parent product for the duration of the `Compute()` pass (see **Parent-chain resolution** below). The sensor's root-folder `DefaultChats.SelectedChats` (the same folder-chats injection that `TreeValuesCache.SendAlertMessage` applies to every alert at delivery time) is folded in **only when the sensor has at least one alert-capable policy** — mirroring `AlertResult.IsValidAlert` (`Template is not null && (Destination.IsFromParentChats || effectiveIsAllChats || resolvedChats.Count > 0)`). `effectiveIsAllChats` is the AND of the parent's `DefaultChats.IsAllChats` and `Destination.IsAllChats` (what `PolicyDestinationHandler.IsAllChats` already returns), NOT `Destination.IsAllChats` alone — checking the raw flag mis-classified a sensor whose only policy is AllChats-mode under a product whose DefaultChats is not All. Fresh sensors have zero policies (the default-policy seeding at `TreeValuesCache.AddSensor` is commented out), so the gating prevents every folder-default chat from being counted for every idle sensor and swamping the badge. Orphan sensors (`Parent == null`) skip the folder lookup entirely because `sensor.Root` casts to `ProductModel` unconditionally inside the getter (`BaseNodeModel.Root`) and would throw. Counts reflect **configuration references**, not active delivery — a disabled policy still counts (the chat is still wired into alert config and the admin should see the blast radius of editing or removing it); blocked / offtime sensors also count (`BaseSensorModel.CanSendNotifications` is not consulted), which is consistent with the configuration-reference framing but worth noting — the badge overstates delivery fidelity slightly for blocked sensors. `AllChats`-mode policies need no special handling for counting: delivery relies on folder-chats injection (the `AllChats` flag is never expanded to concrete ids), so the folder-chats union already covers them once a policy is alert-capable. Compute-on-request, no cache — admin `Index` is low-traffic and the count stays correct after any destination / folder / policy change without invalidation hooks.

  If `Compute()` skips at least one sensor (the cache mutates concurrently with reads and `PolicyDestination.Update` clears `Chats` without a lock — a per-sensor `try/catch` skips and increments a `skipped` counter), every chat's pill renders as `≥N sensors` (the `≥` prefix) with tooltip "Count may be incomplete — at least one sensor was skipped during the scan." A silent partial count on a badge whose sole purpose is judging edit/remove blast radius is worse than no count, so the uncertainty is surfaced rather than logged away. The `ChatViewModel.SensorUsageBadgeText` carries the singular/plural rule and the `≥` prefix; `SensorUsageBadgeTitle` carries the tooltip.

  Parent-chain resolution inside `Policy.GetParentChats` (HSMServer.Core) is a single linear walk that stops at the first ancestor whose `DefaultChats.CurValue.IsFromParent` is false. **Routing change (PR #1327 follow-up to #1310):** the previous implementation walked every strict ancestor UNCONDITIONALLY once `parent.IsFromParent` was true, so on a chain like `root(chats = {C}) → mid(mode = Custom) → leaf(mode = FromParent)` alerts would still reach `root`'s chats — disagreeing with what the destination picker showed the user (`DefaultChatViewModel.GetParentChats` only recurses while each ancestor `IsFromParent`, matching the visible default-chats UI). The new form matches the UI resolution; a non-inheriting intermediate node now breaks the chain the same way in both places. Recipients can only shrink relative to the old behavior, and only on chains deeper than two levels with a non-inheriting middle node — there is no expansion case. The fix also collapses the previous O(2^depth) allocations (the old helper reentered itself once per ancestor) into a linear walk. Pinned by `ChatSensorUsageCalculatorComputeTests.GetParentChats_StopsAtFirstNonInheritingAncestor` and `..._AllAncestorsFromParent_WalksToRoot`.
