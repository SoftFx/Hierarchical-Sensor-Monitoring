...
- `src/tests/HSMServer.Core.Tests/Notifications/ChatSensorUsageCalculatorComputeTests.cs` — `Compute()` entry-point coverage (empty cache → empty counts, `skipped == 0`) plus the calculator's parent-chain pins: `Compute_FromParentUnderNonInheritingMiddle_StopsAtMiddle` (chain `root → mid(Custom, not FromParent) → leaf(FromParent)` resolves only mid's chats, not root's) and `Compute_AllAncestorsFromParent_WalksToRoot` (chain with every ancestor in FromParent mode walks all the way up). These pin `ResolveInheritedChats`'s stop-on-non-inheriting semantics, which is the calculator's own badge-counting rule and does NOT depend on `Policy.GetParentChats` (which delivers alerts and is unchanged by #1310). `Compute_FromParentPolicyWithExtraChats_CountsBothInheritedAndExplicit` pins that a FromParent policy with additional `Destination.Chats` counts both the inherited parent-chain chats AND the explicit extras — a first-class state (alert form JS preserves chats in FromParent mode, alert import does the same, `PolicyDestination.ToString` has a dedicated "from parent chats, {extra}" case) that the calculator used to drop, silently undercounting.
...
- `Views/Configuration/_Chats.cshtml` is the Chats list. After the #1281 rebuild it follows the Members-layout pattern (`Views/Account/Users.cshtml`): a search-by-name input + a channel-filter `<select>` (Any channel / Telegram / Slack / Mattermost; single-select — pick one channel to filter by, or leave on Any to show all) above a `.chat-list` of rows. Each row shows brand icons + chat Name + an Enabled/Disabled badge, with inline Edit (`fa-pen-to-square`, links to `EditChat`) and Remove (`fa-trash-can`, opens `_ConfirmationModal` then POSTs to `RemoveChat`) buttons. Each row also carries a folder-bindings badge cluster between the Name/Enabled badge and the action buttons — up to three `.chat-folder-badge` anchors (click → `FoldersController.EditFolder`) plus a `+N more` popover link when the chat is bound to more than three folders, with a blue `Public` pill for chats with zero bindings (a zero-binding chat is global — appears in every folder's destination picker, see "Folder binding" below; mirrors the Users products-badge pattern, backed by `ChatFoldersViewModel.DisplayFolders` populated in `ChatsSettingsViewModel(IChatsManager, IFolderManager)` post-#1284). Author and Created are no longer shown. Send-test-message actions were removed from the list — they already live on the `EditChat` form (`EditChat.cshtml`) per channel, so Edit is the single entry point for test sends. "Add new chat" links to `AddChat` (admin-only).
  ...
  Parent-chain resolution inside `ChatSensorUsageCalculator.ResolveInheritedChats` (the calculator's own helper, NOT `Policy.GetParentChats`) is a single linear walk that stops at the first ancestor whose `DefaultChats.CurValue.IsFromParent` is false. This matches the destination-picker UI (`DefaultChatViewModel.GetParentChats` recurses while each ancestor `IsFromParent`), so a non-inheriting intermediate node breaks the inheritance for the badge the same way the picker shows it. The walk is memoized once per parent product for the duration of the `Compute()` pass and `AllChats` is ANDed across the visited ancestors (true only when every ancestor has it set). The badge's parent-chain semantics are scoped to badge counting only — `Policy.GetParentChats` (the alert-delivery resolver in HSMServer.Core) is unchanged by #1310 and still walks every strict ancestor once `parent.IsFromParent` is true. Pinned by `ChatSensorUsageCalculatorComputeTests.Compute_FromParentUnderNonInheritingMiddle_StopsAtMiddle` and `..._AllAncestorsFromParent_WalksToRoot`.
