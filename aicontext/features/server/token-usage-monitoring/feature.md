...
- **Attribution**: a request counts for the token whose HsmApiToken identity authenticated it — tokenId/login/EntityId resolved per request from the authoritative stores (`IApiTokenManager.GetToken`, `IUserManager` indexer; index lookups + a projection copy per request; the token principal stays minimal; the EntityId crosses the monitor interface as a Guid, never a string to parse). Every status code counts (403/404/409 are work too); a revocation race (principal authenticated, record gone) attributes nothing and is not an auth failure.
...
- **Lazy subtrees with swept retention**: a token's node is created on the token's first use, and each channel's sensor pair on THAT channel's first use (an unused token — or an unused channel — adds no sensors). Retention is TWO mechanisms: the periodic statistics sweep (`DataCollectorWrapper.UpdateStatictics`) evicts and DISPOSES a node whose token is no longer LIVE — `IApiTokenManager.IsTokenLive`, the sanctioned predicate covering revocation, rotation AND the generation-invalidated window; the node keeps the TokenId in memory for exactly this, never in the tree (#1403 r3: row-existence would have kept a dead token posting `0` req/s for the whole 30-day record retention) — and the sensors' `SelfDestroy` (30 idle days) + `KeepHistory` (7 days) then retire the disposed sensors and bound their history. Dispose is TERMINAL: the collector never un-registers a disposed sensor (the path stays occupied and keeps consuming the sensor budget until process end), so re-creating sensors at an evicted path would hand back the stopped instances and silently drop values. Evicted nodes go to a TOMBSTONE collection keyed by their path (login/entityId): a collector RESTART (the self-monitoring toggle re-initializes every registered sensor) would resurrect the stopped senders — every sweep re-disposes the tombstones, killing any resurrection within one tick — and `NodeFor` refuses a tombstoned path so nothing ever rebuilds on it. The sweep isolates per node, predicate included (one throw retries next tick). Requests cannot drive the eviction: a dead credential fails authentication before any middleware sees its id. Restarts of the SERVER lose nothing (stable paths, re-registration on next use).
- **Renames move the subtree** (#1403 r4): the grouping login is compared on every attribution; a renamed owner's node is tombstoned under the OLD path (its history stays there, like rotation) and a fresh node grows under the current login — the tree always groups a token under a login that exists, at the cost of a history split on rename.
