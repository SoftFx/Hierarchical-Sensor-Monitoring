...
- Live-but-idle tokens are an accepted cost: a `MonitoringRateSensor` posts every post period whether or not anything was counted, so N live tokens ≈ up to 2N points/minute into the self-monitoring product for as long as the tokens exist (`KeepHistory` 7d bounds the stored history). Volume scales with tokens-that-exist, not with API traffic — the `ClientStatisticsSensors` precedent behaves the same per client. An idle-node eviction is a follow-up if it ever matters.
