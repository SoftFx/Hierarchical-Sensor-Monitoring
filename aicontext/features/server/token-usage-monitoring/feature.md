...
- **Pipeline position is load-bearing**: between `UseAuthentication` and `UseAuthorization` — a rejected request never reaches middleware registered after authorization, and the 401s are the failure signal. Pinned in `ManagementPipelineOrderTests`.
