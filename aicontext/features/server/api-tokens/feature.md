...
- Retention removal is `TryRemoveToken`: the durable delete and the in-memory unpublish happen under one state-lock hold, so a concurrent revoke/rotate cannot rewrite a row the removal just deleted and resurrect it after restart. A failed durable removal (`RemoveApiToken` false — the row may still exist) unpublishes nothing. There is deliberately no unpublish-only escape hatch around this ordering.
