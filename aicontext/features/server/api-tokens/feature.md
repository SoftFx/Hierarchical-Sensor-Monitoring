...
- Grants are explicit operation + boundary pairs (Global / Product id / Folder id), canonicalized and deduplicated before persistence; unknown operations, boundary kinds, or resource ids fail closed. Pairs are never recombinable. Grant lists are bounded (`ApiTokenGrants.MaxGrants` = 1024) — canonicalization allocates per grant and the row is rescanned into memory at every boot, so the grant list must not be an unbounded caller-supplied input. Rows are re-canonicalized at load, so the in-memory grant shape always matches the persisted contract.
...
- Free text is bounded and control-character-sanitized before persistence, so records cannot forge log lines: name ≤ 256, description ≤ 1024, revocation reason and the actor fields (created/restricted/rotated/revoked-by) ≤ 256 chars each. Unpaired surrogates are replaced with U+FFFD (System.Text.Json would substitute them in the row anyway), truncation never splits a UTF-16 surrogate pair and never ends in the space of a replaced control character — keeping the live entity byte-identical to the JSON row it round-trips through.
- The live index has an `Unpublish` counterpart to the store's `RemoveApiToken`: retention removes the durable row first and unpublishes only when the removal reported success (`RemoveApiToken` returns false when the row may still exist) — unpublishing after a failed removal, or removing live-first, would resurrect the record after restart.
...
- Generation state: `ApiTokenGeneration_Global` and `ApiTokenGeneration_Owner_<ownerUserId>` hold plain long values; unparsable or negative values are corrupt (the counter is monotonic from 0) and fail the index closed.
