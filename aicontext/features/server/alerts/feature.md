...
- **TTL repeats are CANCELLED per-policy outside the schedule window (#1405)**: `ResendNotification` goes silent AND resets the notification state (`CancelNotification` — the repeat clock and the counter), so a window open with a still-stale value fires a FRESH send at once instead of resuming yesterday's cadence. The fresh send at window open holds for EVERY repeat mode, the DEFAULT `Immediately` included: the never-sent branch answers BEFORE the repeat-mode gate (`Schedule.IsActive` is false for `Immediately`), because on a mixed sensor flipped out-of-window by a schedule-less sibling there is no second sensor transition at window open — the resend arm is the only delivery path left, and gating it on the repeat mode lost the alert forever while the UI kept showing it active (PR #1406 round 2, finding 1). Cancellation is per-policy because the schedule is per-ALERT while OffTime is sensor-wide (never fabricated here); a scheduled policy goes silent in a mixed sensor while its schedule-less siblings keep their cadence, and a per-policy window close sends NO farewell — resolution notifications come only from the sensor-level transition. The two gates are inseparable: the #1404 gate resolves the sensor out-of-window, `GetNotification(false)` nulls the repeat clock, and without the #1405 window arm the "never sent" branch would send on EVERY sweep tick outside the window.
- **One spurious recovery Ok out-of-window is the accepted residual (#1404)**: the RESOLUTION arm of `SetExpiredSnapshot` is deliberately NOT schedule-gated — `GetNotification(false)` resets the state and sends the recovery for every TTL policy. Two paths reach it out-of-window: a restart whose stored newest row is a timeout marker (`LoadHistoryUnderLock` restores `IsExpired = true`, the first out-of-window sweep resolves) and fresh data arriving overnight in a mixed sensor. Operators get a single false "recovered" message while the data is (or was) stale; the alert re-fires at the next window open. Deliberate trade-off: one false-Ok beats the pre-fix alternative (a repeat spam loop into the night), and gating the resolution arm would also suppress genuine overnight recoveries. If a spurious recovery message is unacceptable, the fix is to suppress resolution notifications caused by the window rather than by fresh data — a separate decision, not taken here.
...
`TtlScheduleWindowIntegrationTests.cs` pins the TRANSITION layer on the real cache wiring (real `TreeValuesCache` with `SensorExpired -> SetExpiredSnapshot`, real `AlertScheduleProvider` whose window flips through `SaveSchedule`):
...
- Window closed then OPEN, mixed sensor with the DEFAULT `Immediately` repeat: the cancelled scheduled policy FIRES at the window-open sweep (`RetryCount == 0` via the sweep's resend step reproduced on the real cache wiring) — the first delivery does not live behind the repeat-mode gate.
- Window open: the transition fires the scheduled policy; closing the window and resuming data resolves the sensor and resets the repeat clock (the resolution arm is deliberately ungated — see the residual invariant above).
