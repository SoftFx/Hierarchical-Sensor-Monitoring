...
Coverage for the policy-id index race and its self-heal (#1407) lives in two suites: `src/tests/HSMDatabase.LevelDB.Tests/Policies/PolicyIdsIndexRaceTests.cs` (real LevelDB: concurrent AddPolicy keeps every id indexed; add-vs-remove leaves a consistent pair; `GetPolicy(Guid)` reads a row by direct id, the index-bypassing seam the heal uses) and `src/tests/HSMServer.Core.Tests/Cache/PolicyIndexHealerTests.cs` (the healer unit: orphaned reference with a row is healed into the dictionary under the consumer's RAW reference key, reference with no row anywhere is reported unresolved, already-indexed references are not fetched, duplicate references heal once, duplicate DEAD references are reported and fetched once, unparseable references are left to the load as before).
