...
> Owner: server | Last reviewed: 2026-09-21 | Canonical: yes
...
- **TTL evaluation is schedule-aware at EVALUATION TIME (#1404)**: a TTL policy with an alert schedule gates its expiry decision on `IsWorkingTime(ScheduleId, DateTime.UtcNow)` — never on the stale value's timestamp (the restart incident: a value in-session at session end, no OffTime because the server was down, fired on the first sweep after boot). The gate also RESOLVES the sensor on the first out-of-window sweep (`SetExpiredSnapshot` fires on the transition), so a session close without OffTime ends the alert instead of firing into the night; at window open a still-stale value fires a fresh alert. Schedule-less policies have no gate (unchanged), and in a mixed sensor the sensor-level state follows ANY timing policy — a scheduled policy merely stops contributing out-of-window. The transition arm (`SetExpiredSnapshot`'s notification loop) carries the same gate on the FIRE side: when a schedule-less sibling flips a mixed sensor while the scheduled policy is out-of-window, that policy's alert is CANCELLED silently (state reset, no send) instead of leaking out-of-window — so the mixed-sensor silence holds on the transition path, not only on the repeat path.
- **The window predicate is evaluated ONCE, at one captured instant (#1404)**: `SensorTimeout` computes the per-policy in-window decisions from a single `DateTime.UtcNow` captured at the start of the pass and passes that instant (together with the value it judged) through `SensorExpired` to `SetExpiredSnapshot`, whose notification gate reuses it via `TTLPolicy.IsOutsideSchedule(provider, evaluationTime)` instead of recomputing. A sweep straddling a minute boundary that is also a window boundary must not split the decision from its notification (an alert message with no UI entry, or a UI entry that never sends). The same reasoning forbids "unifying" the TTL gates with the regular data-policy gate in `SensorPolicyCollection`: the data-policy gate is deliberately VALUE-TIME (the value IS the event there — an old value arriving out-of-window is judged by its own timestamp), while the TTL gates are EVALUATION-time (a stale in-session value must not fire when "now" is outside the window). Either unification re-breaks #1404.
- **TTL repeats are CANCELLED per-policy outside the schedule window (#1405)**: `TTLPolicy.TryResendNotification` (a predicate with a hidden write — its out-of-window arm resets the repeat clock and the counter via `CancelNotification`) goes silent AND resets the notification state, so a window open with a still-stale value fires a FRESH send at once instead of resuming yesterday's cadence. The fresh send at window open holds for EVERY repeat mode, the DEFAULT `Immediately` included: for SCHEDULED policies the never-sent branch (null repeat clock) answers BEFORE the repeat-mode gate (`Schedule.IsActive` is false for `Immediately`), because on a mixed sensor flipped out-of-window by a schedule-less sibling there is no second sensor transition at window open — the resend arm is the only delivery path left, and gating it on the repeat mode lost the alert forever while the UI kept showing it active (#1405). The bypass is scoped to scheduled policies because a null clock on them is produced only by the cancellation and resolution paths (both leave the sensor unexpired, so the next expiry re-arms the clock through the transition before the resend loop runs); SCHEDULE-LESS policies keep the master order — the repeat-mode gate answers first, so `Immediately` never delivers from the resend loop for them. Cancellation is per-policy because the schedule is per-ALERT while OffTime is sensor-wide (never fabricated here); a scheduled policy goes silent in a mixed sensor while its schedule-less siblings keep their cadence, and a per-policy window close sends NO farewell — resolution notifications come only from the sensor-level transition. The two gates are inseparable: the #1404 gate resolves the sensor out-of-window, the resolution resets the repeat clock, and without the #1405 window arm the "never sent" branch would send on EVERY sweep tick outside the window (the zombie). Tolerated race, unchanged: the notification state pair (`_lastTTLNotificationTime`, `_notifyCount`) is written from two threads (the sweep via `SetExpiredSnapshot`, the API thread via `TryAddValue -> SensorTimeout`) without a lock — worst case one duplicate notification; the unsynchronized read-modify-write is the established trade-off (cf. the #1296 notes in `BaseSensorModelT`).
- **Window-caused resolutions are SILENT; genuine recoveries still send the Ok (#1404/#1405)**: the resolution arm of `SetExpiredSnapshot` discriminates WHY the sensor resolved. A WINDOW-CAUSED resolution — the policy is out-of-window at the shared evaluation instant AND the evaluated value is still stale (`HasTimeout`: its TTL is up, so the sensor did not recover; the ordinary session close on a single-scheduled-TTL sensor, and the restart whose restored `IsExpired = true` resolves on the first out-of-window sweep) — CANCELS the notification state without sending: no false "recovered" Ok at every session close for a still-silent sensor; the cancelled state fires a FRESH alert at the next window open. The discriminator uses the EVALUATED value, not `sensor.LastValue`: on the data path the event fires before the value reaches Storage, so `LastValue` is still the previous value there. A GENUINE recovery — fresh data arrived, the TTL is no longer up — fails the staleness check and still sends the recovery Ok even out-of-window (a mixed sensor's overnight resume stays visible). Accepted residual, stated plainly: the window-caused resolve still clears `IsExpired`, so the TREE shows the sensor healthy overnight while it is silent — the fresh alert at window open is the operator's signal; keeping the sensor visibly expired out-of-window would require gating the state flip itself, which is not taken here.
...
Coverage for the schedule-window semantics (#1404 + #1405) lives in two suites. `TtlScheduleWindowTests.cs` pins the DECISION layer (`SensorPolicyCollection` / `TTLPolicy`) with a mocked schedule provider that DISCRIMINATES on the timestamp — the stale in-session value always reads in-window, so a value-time gate passes where the evaluation-time gate must not; the incident's exact shape:
