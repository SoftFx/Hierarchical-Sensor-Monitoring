...
- **The policy-id index is race-free and the boot self-heals it (#1407)**: the global policy-id index (one LevelDB key, read-modify-write) is mutated only under a process-wide lock in `EnvironmentDatabaseWorker` — template applies run concurrently on per-product queue threads, and an unlocked interleaving used to lose one side's id forever (the row landed under its own key; the boot path `GetAllPolicies` walks the INDEX, so the load silently skipped the sensor's reference: a template alert that lived in memory until the next restart and vanished with no journal trace). The boot additionally HEALS pre-lock orphans: `PolicyIndexHealer` walks the sensor entities' policy-id references and pulls rows the index missed directly by id (`IDatabaseCore.GetPolicy(Guid)`, the index-bypassing fetch) — existing orphaned rows recover on the first boot after the fix, with no repair tooling and no boot-path writes (the index is left as-is; every boot heals again). A reference that resolves to NO row anywhere is a true orphan: the load still skips it, but logs a Warn with the ids — the difference between "alert vanished without a trace" and a diagnosable boot line.
