...
- **Schedule deletion detaches policy references (#1409)**: `AlertSchedulesController.Remove` awaits `ITreeValuesCache.DetachAlertScheduleFromPoliciesAsync` BEFORE `DeleteSchedule` — every sensor policy (regular and TTL) and every product TTL policy bound to the deleted schedule gets its `ScheduleId` nulled through the standard `PolicyUpdate` full-list path (persisted + journaled, system-force initiator, routed to each entity's ROOT product's queue, mirroring `RemoveChatsFromPoliciesAsync`). The policy itself survives and degrades to schedule-less semantics (always in working time). The detach is best-effort: per-entity failures are logged inside it and the deletion still proceeds (matching `RemoveChatsFromPoliciesAsync`), so a failed detach leaves dangling ids — backstopped by the once-per-id report and fail-open below; detach-first ordering means a CRASH between the two calls leaves the schedule alive, and the operator's `Remove` retry re-runs the idempotent detach. Residual, deliberate: ids that already dangle from BEFORE this change are not retroactively cleaned — `AlertScheduleProvider.IsWorkingTime` still fails open on a missing id (#1405) and reports it ONCE per id per process (a re-saved schedule re-arms the report) instead of one ERROR per scheduled policy per sweep.
...
- Dangling schedule ids created BEFORE #1409 (deletions that predate the detach) persist in stored policies until the operator re-saves the affected policy; they are silent after the once-per-id report and evaluate as always-in-working-time.
