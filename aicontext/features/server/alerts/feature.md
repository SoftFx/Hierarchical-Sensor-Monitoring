...
- **Schedule deletion detaches policy references (#1409)**: `AlertSchedulesController.Remove` no-ops for an unknown id (existence guard before the detach), otherwise awaits `ITreeValuesCache.DetachAlertScheduleFromPoliciesAsync` BEFORE `DeleteSchedule` — every sensor policy (regular and TTL), every product TTL policy, and every alert template policy (regular `Policies` and `TtlEntries`) bound to the deleted schedule gets its `ScheduleId` nulled. Sensor/product entities go through the standard `PolicyUpdate` full-list path (persisted, routed to each entity's ROOT product's queue, mirroring `RemoveChatsFromPoliciesAsync`); templates are patched in place and re-persisted through the `AddAlertTemplate` upsert — a surviving template would otherwise re-mint the dangling id on every new matching sensor and re-attach it on the next template save. TTL intervals are re-asserted explicitly on the detach update (a null `TTL` in full-list semantics is an explicit FromParent reset, so without this every TTL policy on an affected entity would silently lose its interval). The policy itself survives and degrades to schedule-less semantics (always in working time). The detach writes NO journal record, by design of the change journal: it compares each policy's rendered content (`Policy.ToString()`, which does not include `ScheduleId`), and a detach changes no rendered field; the acting operator is also not recorded — the mutation runs under the system-force initiator `DetachAlertSchedule`, which is REQUIRED to reach template-owned policies (a plain user initiator is limited to enable/disable on those). The detach is best-effort: per-entity failures (including template persists) are logged inside it and the deletion still proceeds (matching `RemoveChatsFromPoliciesAsync`), so a failed detach leaves dangling ids — backstopped by the once-per-id report and fail-open below; detach-first ordering means a CRASH between the two calls leaves the schedule alive, and the operator's `Remove` retry re-runs the idempotent detach. Residual, deliberate: ids that already dangle from BEFORE this change (in policies or stored templates) are not retroactively cleaned — `AlertScheduleProvider.IsWorkingTime` still fails open on a missing id (#1405) and reports it ONCE per id per process (a re-saved schedule re-arms the report) instead of one ERROR per scheduled policy per sweep.
