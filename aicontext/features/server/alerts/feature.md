...
- **Schedule deletion detaches policy references (#1409)**: `AlertSchedulesController.Remove` no-ops for an unknown id (existence guard before the detach — but still calls `DeleteSchedule` on that path, which is idempotent, so a stored row the loader SKIPPED because its parse failed is purged instead of being stranded forever: `GetSchedule` reads the in-memory cache only, and such a row never enters it), otherwise awaits `ITreeValuesCache.DetachAlertScheduleFromPoliciesAsync` BEFORE `DeleteSchedule` — every sensor policy (regular and TTL), every product TTL policy, and every alert template policy (regular `Policies` and `TtlEntries`) bound to the deleted schedule gets its `ScheduleId` nulled. Product entities go through the standard `PolicyUpdate` full-list path (persisted, routed to the product's ROOT product's queue, mirroring `RemoveChatsFromPoliciesAsync`); sensors are batched per ROOT product — ONE queue round-trip carries every matching sensor of the branch (one request, one queue pass, though still one entity write per sensor) instead of a round-trip per sensor serialized inside the Remove POST; templates are detached PERSIST-FIRST through the failure-propagating `WriteAlertTemplate` (the detached entity is built WITHOUT touching the live model; only on a successful write are the in-memory ids nulled — a surviving template would otherwise re-mint the dangling id on every new matching sensor and re-attach it on the next template save). TTL intervals are re-asserted explicitly on the detach update (a null `TTL` in full-list semantics is an explicit FromParent reset, so without this every TTL policy on an affected entity would silently lose its interval; an explicit Never/`long.MaxValue` interval degrades to FromParent — the same mapping `TTLPolicy.FullUpdate` applies everywhere, unreachable through the editors). The policy itself survives and degrades to schedule-less semantics (always in working time). The detach writes NO journal record, by design of the change journal: it compares each policy's rendered content (`Policy.ToString()`, which does not include `ScheduleId`), and a detach changes no rendered field — the SAME rendered-content discriminator now also guards the change table (`BaseNodeModel.Update` re-stamps a TTL policy's recorded owner only when the update actually mutated the policy), so a full-list re-assertion does NOT transfer ownership: a hand-edited User-owned TTL policy on an affected entity keeps its owner across a detach, and a later template apply stays blocked by the `CanChange` gate instead of silently overwriting it. The acting operator is also not recorded — the mutation runs under the system-force initiator `DetachAlertSchedule`, which is REQUIRED to reach template-owned policies (a plain user initiator is limited to enable/disable on those). The detach is best-effort: per-entity failures are logged inside it with schedule/product/sensor context (queue results are checked per arm; per-policy soft errors on the sensor arm are logged too; a failed template persist is logged and leaves that template's memory untouched so the operator's retry re-runs it) and the deletion still proceeds (matching `RemoveChatsFromPoliciesAsync`), so a failed detach leaves dangling ids — backstopped by the once-per-id report and fail-open below; detach-first ordering means a CRASH between the two calls leaves the schedule alive, and the operator's `Remove` retry re-runs the idempotent detach. Residual, deliberate: ids that already dangle from BEFORE this change (in policies or stored templates) are not retroactively cleaned — `AlertScheduleProvider.IsWorkingTime` still fails open on a missing id (#1405) and reports it ONCE per id per process (a re-saved schedule re-arms the report) instead of one ERROR per scheduled policy per sweep. Residual, accepted: an operator's template save can interleave with the detach (`AddAlertTemplateAsync` REPLACES the dictionary entry rather than mutating the live model, and whichever persist lands last wins), so the save can restore a dangling id — the once-per-id report surfaces it and re-running the schedule delete re-runs the detach.
...
- Dangling schedule ids created BEFORE #1409 (deletions that predate the detach) persist in stored policies and stored alert templates until the operator re-saves the affected entity; they are silent after the once-per-id report and evaluate as always-in-working-time.
