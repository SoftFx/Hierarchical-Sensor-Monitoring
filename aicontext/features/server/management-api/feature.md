...
|---|---|---|
| `GET /api/v1/alertTemplates?page&pageSize` | `AlertTemplatesApiController.GetTemplates` | 200 `{items, page, pageSize, totalCount, totalPages}`; clamps page≥1, 1≤pageSize≤200 (default 50); order: name (OrdinalIgnoreCase), then id |
...
- List re-resolves the caller per item (`IsVisible`); a page can straddle a mid-request grant/role change. A snapshot API in the evaluator is a possible follow-up.
