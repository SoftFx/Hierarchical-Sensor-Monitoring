...
- Root product: `type=product`, empty `parent`, children split folders/sensors ordered by name, sensor type names; nested folder: `type=folder`, parent ref, path `root/folder`; a node's `path` is its FULL path (a root's path is its name, unlike the model's empty `Path`); a FOLDER id authorizes at the ROOT product boundary (pinned: the evaluator is asked for the root id, never the folder id).
