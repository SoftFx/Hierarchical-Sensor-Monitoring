...
- **What changed:** `ShouldDestroy()` defers the decision (returns `false`) unless `IsHistoryLoaded` is true — meaning "history actually loaded", not "a load was attempted" (a failed load latches `_isInitialized` but never publishes; for such sensors self-destroy is disabled until restart, logged at `Warn` by the sweep). With an empty `Storage` cache (retention purge, or the newest row is the `SetExpiredSnapshot` marker, which never enters the cache) it falls back to `LastTimeout?.Time` before `CreationDate` — deliberately conservative: the marker's time over-estimates last activity by up to one TTL. It stays a pure predicate: no history load, no policy fan-out, no notifications.
- **What it relies on:** `ClearDatabaseService.ServiceActionAsync` awaits `CheckSensorsHistoryAsync` (which initializes every sensor, with per-sensor error isolation) before the self-destroy sweeps — pinned by `ClearDatabaseServiceTests`. The sweep logs a deferred count, and `CheckSensorsHistoryAsync` logs per-product `TaskResult` failures instead of discarding them.
- **Still open:** sensors registered after the history check in the same tick are deferred for one sweep; `HasData` remains a cache-occupancy signal used as "sensor ever had data" — acceptable until a concrete consequence appears. A failed load is never retried, so a transient database error during a *lazily* triggered `Initialize()` (first incoming value) disables self-destroy for that sensor until restart even after it resumes reporting. And the `LastTimeout` fallback is in-memory only: after a restart it survives only if the timeout-marker row still exists in the database — a sensor whose marker row was purged by retention falls back to `CreationDate` again (same data, different outcome depending on process lifetime).
