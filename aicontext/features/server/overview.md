...
- **Failed-load retry (#1344):** the `Initialize()` failure latch (anti-retry-storm, #1296) is rerun by `BaseSensorModel<T>.RetryFailedHistoryLoad(utcNow)` — called only from the self-destroy sweep, never from the per-value paths. Timing has two gates: the first retry must clear one sweep period measured from the **failure** itself (so the sweep that just observed the failure — including the eager `Initialize()` of `CheckSensorsHistoryAsync` one await earlier in the same maintenance tick — never hits the DB back-to-back, and a transient outage costs ~1h, not 24h); later retries are spaced 24h from the previous retry. The rerun mode is explicit (`LoadHistoryUnderLock(isRetry)`), because a live sensor can have a null `LastValue` (timeout-only traffic, rejected values, retention purge) and "Storage looks empty" does not mean "no live ingestion": a retry is **write-free except `LastTimeout`** (newest-wins marker restore, which `ShouldDestroy()`'s empty-cache fallback keys on) **and `Cut(From)`**. It does not rebuild the value cache, does not restore `IsExpired`, does not re-arm TTL clocks and runs no policy fan-out — a retried sensor's self-destroy decision rides on the live cache, the restored marker, `Storage.To` or `CreationDate`, which can destroy up to one interval earlier than a cleanly-loaded twin when the cache is empty and the newest DB row is not a marker.
