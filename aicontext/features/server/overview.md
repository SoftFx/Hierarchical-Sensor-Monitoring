...
- **What changed:** `ShouldDestroy()` defers the decision (returns `false`) unless `IsHistoryLoaded` is true — meaning "history actually loaded", not "a load was attempted" (a failed load latches `_isInitialized` but never publishes; for such sensors self-destroy is disabled between the bounded retries below, logged at `Warn` by the sweep). The decision is the **newest of every activity signal**, never a priority chain and never a branch that hides one of them: `LastUpdate` (only when `HasData`), the timeout marker's `.Time`, and `Storage.To` (the newest of the last real value's time — `Clear()` does not reset it — and the floor a history-load retry restored), falling back to `CreationDate` only when the sensor never received a value. Newest-of matters in both directions: `_lastTimeout` is not advanced once a newer real value arrives (a later re-expiry writes no marker, because `SetExpiredSnapshot` requires `HasData`), so a stale marker must not shadow a fresher `To`; and a cached value must not shadow a newer marker or floor, because on a sensor whose cache is empty `AddValueBase` accepts the next value whatever its timestamp (the newest-wins guard has no `_lastValue` to compare against), so one out-of-order value — a reconnecting collector flushing a stale queue — used to flip `HasData` and hide a newer signal behind its own old `LastUpdate`. The marker path is deliberately conservative: the marker's time over-estimates last activity by up to one TTL. `ShouldDestroy()` stays a pure predicate: no history load, no policy fan-out, no notifications. The sweep itself is self-sufficient for never-attempted sensors: it calls `Initialize()` on any self-destroy-enabled sensor that `CheckSensorsHistoryAsync` never reached (e.g. sensors lost to a product-name or sensor-path collision), with per-sensor exception isolation.
...
- **Failed-load retry (#1344):** the `Initialize()` failure latch (anti-retry-storm, #1296) is rerun by `BaseSensorModel<T>.RetryFailedHistoryLoad(utcNow)` — called only from the self-destroy sweep, never from the per-value paths, and at most `MaxHistoryLoadRetriesPerSweep` times per sweep (a whole-database outage latches many sensors at once; the cap keeps the serial sweep from turning into a synchronized burst of throwing reads — capped-out sensors are picked up by the next sweep and lose no retry budget). The cap counts only retries that reached the database **and failed again**: `RetryFailedHistoryLoad` returns whether it ran, and neither a sensor waiting out its backoff nor a successful post-recovery retry is charged. Charging suppressed calls would let the sensors early in the (stable) enumeration order exhaust the cap sweep after sweep and permanently starve every sensor past it; charging successful ones would make a large latched set drain at 100 sensors per hour, though each of those retries is an ordinary read pair costing milliseconds. Timing has two gates, both on the caller-supplied clock: the first retry must clear one sweep period measured from the **failure** itself (so the sweep that just observed the failure — including the eager `Initialize()` of `CheckSensorsHistoryAsync` one await earlier in the same maintenance tick — never hits the DB back-to-back); later retries back off from the previous retry — 2h, 4h, 8h, 16h, then a 24h cap. The first delay is one full sweep period rather than a fraction of one, so it holds however long the eager history check takes on a large tree; the cost is that a failure mid-interval waits up to 2h for its first retry. After that an outage degrades gradually instead of jumping straight to a full day of disabled self-destroy, and the cap keeps a permanently broken database at one attempt per sensor per day (#1296). `HistoryLoadFirstRetryDelay` and `ClearDatabaseService.Delay` are both 1h and must be kept in step. The rerun mode is explicit (`LoadHistoryUnderLock(isRetry, utcNow)`), because a live sensor can have a null `LastValue` (timeout-only traffic, rejected values, retention purge) and "Storage looks empty" does not mean "no live ingestion": a retry is **write-free except for the restored activity signal** — `LastTimeout` (newest-wins marker restore) when the newest DB row is a timeout marker, or the `Storage.To` activity floor (`SetLastActivity`, kept in its own field and merged into `To` at read time, so it can be outgrown by live ingestion but never overwritten by it) when it is a real value; without the `To` floor a non-marker newest row would restore nothing, the latched load would let `ShouldDestroy()` fall through to `CreationDate`, and an established sensor whose newest value is minutes old would be deleted — **and `Cut(From)`**. The floor is stamped without validating the row, unlike the cold load, which skips a policy-rejected newest row — so a retried sensor can look active where its cleanly-loaded twin would fall back to `CreationDate` (conservative: it destroys later, never earlier). It does not rebuild the value cache, does not restore `IsExpired`, does not re-arm TTL clocks and runs no policy fan-out — a retried sensor's self-destroy decision rides on the live cache, the restored marker or the restored `To`: with an empty cache it judges on the newest DB row just like a cleanly-loaded twin, except via the marker's `.Time`, which over-estimates last activity by up to one TTL and so destroys later, never earlier.
