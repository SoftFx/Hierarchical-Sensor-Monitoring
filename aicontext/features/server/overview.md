...
Direct `Storage` readers stay unguarded, with one now-guarded exception: `ShouldDestroy()` (#1328). What changed: it defers the decision (returns `false`) unless `IsInitialized` is true — which means "history actually loaded", not "a load was attempted" (a failed load latches `_isInitialized` but never publishes) — and when the `Storage` cache is empty (retention purge, or the newest row is the `SetExpiredSnapshot` marker, which never enters the cache) it falls back to `LastTimeout?.Time` before `CreationDate`. It stays a pure predicate: no history load, no policy fan-out, no notifications. What it relies on: `ClearDatabaseService.ServiceActionAsync` awaits `CheckSensorsHistoryAsync` before the self-destroy sweep, and `CheckSensorsHistoryAsync` logs per-product failures instead of discarding them (a swallowed failure would leave sensors deferred — the sweep's log line reports the deferred count). Still open: sensors registered after the history check in the same tick are deferred for one sweep, and `HasData` remains a cache-occupancy signal used as "sensor ever had data" — acceptable until a concrete consequence appears. Verdict on the remaining unguarded readers (`LastValue`, `LastDbValue`, `HasData` elsewhere, `LastUpdate`, `From`/`To`, `Result`, `Revalidate()`, `Cut()`, `Clear()`, #1325 inventory): none destroys data — they at worst show an empty/stale view during the startup window or act on it (e.g. a TTL revalidation against no last value is a no-op).
