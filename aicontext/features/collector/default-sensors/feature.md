...
- **На простаивающем хосте оценка расходится за десятки минут, не мгновенно.** Конкретно: после
  всплеска в 1 MiB/s, оставившего оценку в 910 с на 910 MiB свободного места, каждые 30 с простоя
  умножают скорость на 0.9 (половина за ~3.3 мин). Порог алерта `value <= 2 дней` такой хост
  перестаёт пробивать примерно через 50 замеров (~25 минут), а потолка достигает через ~99 замеров
  (~50 минут). До #1445 он не переставал никогда — это и была жалоба владельца, но "десятки минут" —
  это не "сразу".
