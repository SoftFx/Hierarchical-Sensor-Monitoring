...
- Special options (`Options/SpecialSensorOptions.cs`): `DiskSensorOptions { TargetPath default C:\, CalibrationRequests default 6 (замеров, не постов — #1445), PostDataPeriod 5 min, internal SpaceCheckPeriod 30 s }`, `DiskBarSensorOptions { TargetPath }`, `VersionSensorOptions { Version, StartTime }`, `ServiceSensorOptions { ServiceName, IsHostService default true (→ .module placement), SensorPath }`, `NetworkSensorOptions`, `WindowsInfoSensorOptions { PostDataPeriod default 12 h }`, `CollectorMonitoringInfoOptions`.
