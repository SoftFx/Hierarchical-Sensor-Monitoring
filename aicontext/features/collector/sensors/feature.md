...
- Time alignment (`Extensions/BarTimeExtensions.cs`): `OpenTime = floor(UtcNow.Ticks / period) * period` (UTC-epoch aligned), `CloseTime = OpenTime + BarPeriod`. The send loop's due time is `GetTimerDueTime(PostDataPeriod)` — the next wall-clock multiple of `PostDataPeriod` (a full period when now is exactly on one) — so partial posts land on the post grid; the collect loop starts `BarTickPeriod` after Start and rolls the bar on the first tick with `CloseTime < now` (strict).
- Periods: `BarPeriod` 5 min, `BarTickPeriod` 5 s, `PostDataPeriod` 15 s, `Precision` 2 (0–15); all validated > 0.
- **Partial posts**: every `PostDataPeriod` post is a copy of the in-progress bar with the bar's own `OpenTime`/`CloseTime`; the server overwrites same-`OpenTime` posts and persists one bar per `BarPeriod`. The native collector mirrors this for ALL its built-in bars since 0.7.1 — metric-driven default bars, queue diagnostics, network speed (#1428, [`default-sensors/feature.md`](../default-sensors/feature.md#native-metric-driven-bars-partial-posts-1428)); native **public** bars (`CreateDoubleBarSensor` & co.) still roll on add and publish only the closed bar and the stop flush — their `post_period_ms` is accepted but not scheduled.
...
- Special options (`Options/SpecialSensorOptions.cs`): `DiskSensorOptions { TargetPath default C:\, CalibrationRequests default 3 (замеров, не постов — #1445), PostDataPeriod 5 min, internal SpaceCheckPeriod 10 min }`, `DiskBarSensorOptions { TargetPath }`, `VersionSensorOptions { Version, StartTime }`, `ServiceSensorOptions { ServiceName, IsHostService default true (→ .module placement), SensorPath }`, `NetworkSensorOptions`, `WindowsInfoSensorOptions { PostDataPeriod default 12 h }`, `CollectorMonitoringInfoOptions`.
