...
- **Startup:** `lb_try_duration 30s` in the upstream snippet makes Caddy wait for HSM (database load) instead of answering 502. Retries cover dial failures, so no request is sent twice.
