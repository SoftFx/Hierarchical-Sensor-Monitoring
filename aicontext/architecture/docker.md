...
- **Recovery when Caddy is down:** `app` publishes nothing, so Caddy is a single point of failure for the UI **and** for ingestion. The documented `docker-compose.recovery.yml` override (`127.0.0.1:44333`, optional `44330`, trust switched off) is the way into HSM while the proxy does not start (`wiki-git/Installation.md`, "If Caddy does not start").
...
- **Certificate fallback (#1431):** the `HSM_DOMAIN` sites import `(hsm_tls)` with `issuer acme` then `issuer internal`. If Let's Encrypt refuses or cannot validate, Caddy issues from its own CA instead of leaving every handshake without a certificate. That would take down the UI and ingestion, since `default_sni`/`fallback_sni` route every client to this certificate. Internal certificates live 12 h, so ACME is retried on each renewal. Verified live: `HSM_DOMAIN=hsm.invalid` gets `rejectedIdentifier` from Let's Encrypt, then an internal certificate, and all paths are served within seconds. For an IP `HSM_DOMAIN`, Caddy skips ACME and uses the internal CA directly.
  - **Limits of the fallback.** A rejection falls back in seconds. A validation timeout (port 80 filtered, DNS not ready) takes minutes first, and there is no certificate for `default_sni` meanwhile. Browsers that ever loaded the UI with the Let's Encrypt certificate have HSTS cached (`AddHsts`: 30-day max-age, `includeSubDomains`, `Preload`). They refuse the internal certificate with no click-through, so on a renewal failure the web UI is browser-inaccessible while ingestion from allow-untrusted collectors continues. #1431 item 5 (HSTS preload) must be settled with this in view.
  - Listing issuers explicitly drops no public CA. Without an e-mail, Caddy 2.11.4's default is Let's Encrypt only; verified by letting a default site fail, which tried `acme-v02.api.letsencrypt.org` only.
