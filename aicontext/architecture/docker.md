...
- **Client IP (#1427):** HSM restores it from Caddy's `X-Forwarded-For` through `UseForwardedHeaders`, registered first in `ConfigureMiddleware`, only when `Kestrel.TrustedProxies` is non-empty. It uses `XForwardedFor` only (the upstream hop is HTTPS already, so the scheme is never rewritten), `ForwardLimit = 1`, and the framework's implicit loopback trust cleared.
  ...
  - `TrustedProxies` is deployment-owned and read **from the environment only** (`KestrelConfig.ReadTrustedProxies`, blank entries dropped). A copy in `appsettings.json` is ignored with a startup warning (`SettingsFileHasTrustedProxies`), and `[JsonIgnore]` keeps `ResaveSettings` from writing one. A persisted copy would outlive the deployment that needed it; e.g. a later `docker run` with published ports would trust its bridge gateway, where userland-proxied connections come from.
  ...
  - `attached-networks` includes the bridge gateway. That is harmless while `app` publishes nothing. The recovery override publishes a port, so it also sets `Kestrel__TrustedProxies__0: ''`. On `network_mode: host` or macvlan the keyword would trust the whole LAN, so it is meant only for the bundled compose network.
