...
| MCP surface (`/mcp`) | The read-only Model Context Protocol endpoint hosted by HSMServer (#1391): Streamable HTTP, nine snake_case tools rendering the management-API read surface through the SAME HsmApiToken credential and owner sight. | Outside the `/api/v1` area guard (JSON-RPC errors, SDK-owned handler) but inside the bearer credential family; SitePort-only. See `aicontext/features/server/mcp/`. |
| Token usage sensors | The per-token view of management-API usage inside self-monitoring: how often the token authenticates `/api/v1`/`/mcp` requests and how long they run. Keyed by owner login + token EntityId — never the name (collides, moves) or the TokenId (never disclosed). | The Profile token card displays the EntityId as the join key. See `aicontext/features/server/token-usage-monitoring/`; ADR `docs/decisions/`-style rationale in `docs/adr/0001`. |
