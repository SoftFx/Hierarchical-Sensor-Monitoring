...
| Operation family | The resource prefix of a management operation (`products` in `products:read`); the grant picker's table rows group operations by it. | Naming discipline per `ApiTokenOperations` (ADR-0002); read/write are always separate operations of one family. |
