...
9. **Collector bugs are parity bugs** -> the DataCollector exists in two implementations (.NET `src/collector/`, native C++ `src/native/` — epic #1093). A bug found in EITHER implementation must be triaged against the other before closing: write the reproducing conformance scenario (`tests/conformance/*.hsmtest`) FIRST, run it against both collectors, then fix every implementation it is red on. One issue covers both fixes (labels `bug` + `cpp-port`) and must record the per-implementation verdict: "fixed in both" / "only X affected because ..." / "intentional divergence" (divergences are documented in `aicontext/features/api/wire-contract/feature.md` or the relevant feature doc)
