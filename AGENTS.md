...
9. **Collector behavior changes ship with conformance coverage** -> portable observable behavior of DataCollector (any language) lands with a `tests/conformance/collector/*.hsmtest` scenario plus verb support in BOTH drivers (.NET + native) in the same PR; review fails without it. DSL spec: `tests/conformance/README.md`; parity-bug flow: epic #1093
