...
namespace HSMDataCollector.Prototypes
{
    ...
    {
        private const string CalibrationInfo = "After the start of the sensor, it's calibrated during {0} free-space measurements (one every 10 minutes) that post with OffTime status. Only an Ok status carries a real estimate; every other status posts the 365-day ceiling, which means there is no actionable estimate.";
