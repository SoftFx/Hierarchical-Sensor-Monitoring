    ...
    public sealed class CollectorSensorCardinalityStressTests
    {
        ...
        public void Cardinality_registration_repeated_for_duration_stays_bounded()
        {
            var sensorCount = GetPositiveIntEnvironment(
                ...
                new CollectorOptions().MaxSensors);
            var duration = GetDurationEnvironment("HSM_COLLECTOR_CARDINALITY_NIGHTLY_SECONDS", TimeSpan.FromSeconds(5));
            var maxDuration = GetDurationEnvironment("HSM_COLLECTOR_CARDINALITY_NIGHTLY_MAX_SECONDS", TimeSpan.FromSeconds(20));
