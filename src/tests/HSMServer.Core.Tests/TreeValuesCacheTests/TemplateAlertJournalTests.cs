...
namespace HSMServer.Core.Tests.TreeValuesCacheTests
{
    ...
    public class TemplateAlertJournalTests : MonitoringCoreTestsBase<TemplateConcurrencyFixture>
    {
        ...
        private List<JournalRecordModel> CaptureJournal(Action action)
        {
        ...
        public async Task RemoveTemplate_TtlPolicies_LeaveJournalRecords()
        {
            ...
            var records = CaptureJournal(() =>
                Assert.True(_valuesCache.RemoveAlertTemplateAsync(template.Id).Result.Success));
        ...
        public async Task UpdateTTLs_ManualDrop_LeavesJournalRecord()
        {
            ...
            var records = CaptureJournal(() => sensor.Policies.UpdateTTLs([], initiator));
        ...
        public async Task ApplyTemplate_OrphanOnlyTtlRemoval_IsPersisted()
        {
            ...
            // Persisted: UpdateSensor ran for the sensor's product even though
            // the update lists were empty and nothing matched.
            Assert.Contains(_fixture.ProductAId.ToString(), _persistedProductIds);
        }
