...
3. **`/api/v1` is a fail-closed area** enforced by `ManagementApiGuardMiddleware` before authentication: an endpoint is reachable only on the SitePort listener (via the immutable `HsmListenerBindings` registry, the same instance that drove `Listen`) and only when it carries `[ManagementApi]`, is not anonymous, and requires the management policy (the reserved cookie-only `/api/v1/api-tokens` family is the documented exception). Everything else in the area is a plain 404 — a newly added route without the metadata is unreachable by default, and 404 (not 403) never confirms a route's existence on the wrong port.
