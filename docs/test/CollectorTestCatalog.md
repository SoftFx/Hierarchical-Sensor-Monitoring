...
| Adversarial lifecycle | 40 fast cases + 1 gated repeat | 97% | ~4-6 sec быстрый; gated suite repeat 30 sec default | `src/collector/HSMDataCollector.Tests/CollectorAdversarialTests.cs` | [CollectorAdversarialTests.md](CollectorAdversarialTests.md), [CollectorBreakCandidates.md](CollectorBreakCandidates.md), [CollectorSuiteSoakTests.md](CollectorSuiteSoakTests.md) |
| Crash isolation (process-isolated) | 5 fast + 11 listener matrix | 85% | ~3-4 sec (spawned processes) | `CollectorCrashIsolationTests.cs`, `ProcessEventListenerTests.cs`, `src/collector/HSMDataCollector.CrashTests.Host/` | Раздел `Crash Isolation` ниже |
| Reliability wave #1102 (E1/B2/C1/E2/E4/E5) | 36 fast | 85% | ~2 sec | `PerformanceCounterInstanceResolutionTests.cs`, `BoundedBlockingCallTests.cs`, `FileSensorBoundsTests.cs`, `RateSensorElapsedTimeTests.cs`, `TransportAndTimeNormalizationTests.cs` | aicontext `features/collector/overview.md`, раздел `Reliability hardening`. Сверх red-first юнитов: E1 reshuffle-churn 100 циклов + loud-failure/recovery; C1 JSON-эквивалентность zero-copy списка, LINQ-семантика, SendFile cap-reject и байтовый roundtrip через pipeline; E2 конкурентная консервация суммы (4 writers x 10k); B2 параллельные bounded-вызовы; E5 end-to-end до границы IDataSender |
...
| Пользовательский `IDataSender` игнорирует cancellation | 1 | 80% | <1 sec | `Stop_with_data_sender_that_ignores_cancellation_does_not_hang` | `CollectorAdversarialTests.cs` | [CollectorAdversarialTests.md](CollectorAdversarialTests.md), таблица `Какие сценарии проверяются`; [CollectorBreakCandidates.md](CollectorBreakCandidates.md), раздел `Детали кандидата с sender cancellation` |
| Priority sender игнорирует cancellation | 1 | 80% | <1 sec | `Stop_with_priority_sender_that_ignores_cancellation_does_not_hang` | `CollectorAdversarialTests.cs` | [CollectorAdversarialTests.md](CollectorAdversarialTests.md), таблица `Какие сценарии проверяются` |
| Command sender игнорирует cancellation | 1 | 80% | <1 sec | `Stop_with_command_sender_that_ignores_cancellation_does_not_hang` | `CollectorAdversarialTests.cs` | [CollectorAdversarialTests.md](CollectorAdversarialTests.md), таблица `Какие сценарии проверяются` |
| File sender игнорирует cancellation | 1 | 80% | <1 sec | `Stop_with_file_sender_that_ignores_cancellation_does_not_hang` | `CollectorAdversarialTests.cs` | [CollectorAdversarialTests.md](CollectorAdversarialTests.md), таблица `Какие сценарии проверяются` |
...
| Concurrent exception deduplication | 1 | 75% | <1 sec | `Message_deduplicator_handles_concurrent_duplicate_bursts` | `CollectorAdversarialTests.cs` | [CollectorAdversarialTests.md](CollectorAdversarialTests.md), таблица `Какие сценарии проверяются` |
...
| Создание сенсора во время `Stop()` | 1 | 80% | <1 sec | `Creating_sensor_while_stop_is_in_progress_does_not_start_it` | `CollectorAdversarialTests.cs` | [CollectorAdversarialTests.md](CollectorAdversarialTests.md), таблица `Какие сценарии проверяются` |
| Параллельная регистрация одного sensor path | 1 | 80% | <1 sec | `Concurrent_sensor_registration_for_same_path_returns_existing_sensor` | `CollectorAdversarialTests.cs` | [CollectorAdversarialTests.md](CollectorAdversarialTests.md), таблица `Какие сценарии проверяются` |
...
| Короткий in-flight timer callback при `Stop()` | 1 | 80% | <1 sec | `Stop_waits_for_short_function_timer_callback_before_disposing_sensor` | `CollectorAdversarialTests.cs` | [CollectorAdversarialTests.md](CollectorAdversarialTests.md), таблица `Какие сценарии проверяются` |
| Restart после timeout остановки queue worker-а | 1 | 85% | <1 sec | `Start_after_stop_timeout_does_not_mark_collector_running_until_queues_finish` | `CollectorAdversarialTests.cs` | [CollectorAdversarialTests.md](CollectorAdversarialTests.md), таблица `Какие сценарии проверяются` |
| Plaintext transport opt-in для явного `http://` | 1 theory / 2 cases | 85% | <1 sec | `Explicit_http_server_address_requires_plaintext_opt_in` | `CollectorAdversarialTests.cs` | [CollectorAdversarialTests.md](CollectorAdversarialTests.md), таблица `Какие сценарии проверяются` |
| Throwing `ExceptionThrowing` subscriber не останавливает sensor loop | 1 | 85% | <1 sec | `Throwing_ExceptionThrowing_subscriber_does_not_stop_sensor_loop` | `CollectorAdversarialTests.cs` | host-callback matrix (#1103); процессная половина — `CollectorCrashIsolationTests` |
| Throwing `ExceptionThrowing` subscriber не блокирует остальных подписчиков | 1 | 85% | <1 sec | `Throwing_ExceptionThrowing_subscriber_does_not_starve_other_subscribers` | `CollectorAdversarialTests.cs` | host-callback matrix (#1103) |
| Throwing `onError` callback не убивает scheduler | 1 | 85% | <1 sec | `Throwing_onError_callback_does_not_kill_scheduler_or_other_tasks` | `CollectorAdversarialTests.cs` | host-callback matrix (#1103); процессная половина — `CollectorCrashIsolationTests` |
| Throwing custom `ICollectorLogger` | 1 | 80% | <1 sec | `Throwing_custom_logger_does_not_escape_collector_operations` | `CollectorAdversarialTests.cs` | host-callback matrix (#1103) |
| Throwing `ILifecycleListener` не ломает transitions и других listeners | 1 | 80% | <1 sec | `Throwing_lifecycle_listener_does_not_affect_transitions_or_other_listeners` | `CollectorAdversarialTests.cs` | host-callback matrix (#1103) |

## Crash Isolation (process-isolated, #1103)

Назначение: доказать инвариант «host callbacks never crash the host» для краш-векторов #1102 A1/A2. Краш хост-процесса нельзя заассертить изнутри xUnit-процесса, поэтому `CollectorCrashIsolationTests` спавнит консольный хост `HSMDataCollector.CrashTests.Host` (net6.0, `RollForward=LatestMajor`) со сценарием, который подключает заведомо бросающий callback и триггерит вектор. Хост печатает `HOST_SURVIVED` и выходит с кодом 0 только если изоляция держит; до фикса инжектированное исключение убивало процесс до sentinel (red), после фикса — green.

| Что покрывает | Тестов | Длительность | Сценарий хоста | Где код |
| --- | ---: | --- | --- | --- |
| A1: throwing `ExceptionThrowing` subscriber при ошибке monitoring-сенсора (async-void dispatch) | 1 | ~2 sec | `a1-throwing-exception-subscriber` | `CollectorCrashIsolationTests.cs`, `HSMDataCollector.CrashTests.Host/Program.cs` |
| A1: throwing `onError` на seam шедулера (`CollectorScheduler.Schedule`) | 1 | ~1.5 sec | `a1-throwing-onerror` | там же |
| A2: malformed EventCounters payloads через настоящий `EventSource` → `EventListener` dispatch; после malformed событий доставка `time-in-gc` продолжается | 1 | <1 sec | `a2-etw-malformed-counters` | там же + `FakeRuntimeEventSource.cs` |
| A2: true ETW-path smoke — well-formed `time-in-gc` доходит до `OnTimeInGC` | 1 | <1 sec | `a2-etw-time-in-gc-smoke` | там же |
| Контроль: неизвестный сценарий хоста = exit 64, не vacuous pass | 1 | <1 sec | `no-such-scenario` | `CollectorCrashIsolationTests.cs` |

Unit-матрица malformed-payload'ов для извлечённого тела коллбэка (`ProcessEventListener.ProcessEventCounters`): `ProcessEventListenerTests.cs`, 11 тестов (null `EventName`, null payload, отсутствующие `Name`/`Mean`, non-double `Mean`, изоляция последующей доставки, happy path).

Эмпирика (net6, зафиксировано при red-репродукции): in-proc `EventSource` dispatch глотает исключения listener-коллбэков (`ThrowOnEventWriteErrors=false`), поэтому неогороженный A2-listener терял counter values молча, а не ронял процесс; guard убирает зависимость от этого поведения рантайма.

CI: `.github/workflows/collector-unit-tests.yml` (windows runner, весь unit suite включая обе lane).
