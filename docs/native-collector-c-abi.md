...
- **0.8.1** (#1445) — no ABI change. The disk-space prediction math behind
  ...
  rewritten (signed drain EMA, five explicit posted states, a 365-day ceiling, calibration counted
  on the sampling clock) to mirror the managed fix. A host that only links the ABI sees the same
