...
9. **Collector behavior changes ship with conformance coverage** -> portable observable behavior of DataCollector (any language) lands with a `tests/conformance/collector/*.hsmtest` scenario plus verb support in BOTH drivers (.NET + native) in the same PR; review fails without it. DSL spec: `tests/conformance/README.md`; parity-bug flow: epic #1093. CI-enforced (#1101): the `checklist-gate` job runs `scripts/conformance-coverage.ps1 -Strict` (every functional-checklist line must carry a disposition; every `— conformance:` ref must resolve to a real corpus case) and `scripts/conformance-unsupported-triage.ps1` (every `CONFORMANCE-UNSUPPORTED` marker must reference a cpp-port issue). The functional checklist `docs/initiatives/cpp-collector-port-functional-inventory.md` is a **frozen index** — the conformance corpus is the source of truth; do not reopen boxes there, add the scenario instead.
10. **One sensor = one acquisition mechanism (no per-language sources)** -> if a default sensor exists under the same path in both collectors, both MUST read the same OS data source and run a mirrored algorithm; do NOT use a different per-language facade (e.g. PDH/performance counters in .NET vs a Win32 API in native) — divergent normalization of names/values breaks byte-identical parity. The managed side may use a .NET wrapper over the same source (e.g. `System.Diagnostics.Process` over `GetProcessTimes`; `System.Net.NetworkInformation.NetworkInterface` over `GetIfTable2`). Lock equivalence with a conformance scenario per rule #9. Precedent: top-CPU (`cpu_top.cpp` ↔ `WindowsTopCpuMonitor.cs`).
...
- **Review your own diff before pushing it.** After committing and before pushing, run `git fetch origin master`, then `/code-review medium origin/master...HEAD` (or `/matt:code-review` for the standards + spec axes) on the whole committed branch diff; the range sees committed work only. Adjudicate the findings, fix what is real, commit and re-verify the fixes, then push. Re-review only when a fix goes beyond its finding; new work added after the review needs a review of its own. The PR bot (`claude-review.yml`) is the second reader, not the first: every round of it is a paid API call plus a full rebuild/verify/push cycle, and its rounds are worth spending on what a local pass could not see.
