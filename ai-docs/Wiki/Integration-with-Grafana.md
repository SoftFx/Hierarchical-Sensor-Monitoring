...
HSM supports the ability to integrate its data with the Grafana service. This allows you to build custom dashboards and visualize sensor data using Grafana's powerful charting capabilities.

See also: [Supported Grafana Datasources](Supported-Grafana-datasources), [Access Keys](Access-keys), [Sensor Types](Sensor-types)

---

![image](https://github.com/SoftFx/Hierarchical-Sensor-Monitoring/assets/43994777/ac9e139e-1769-4034-971b-b670c2915d87)

In order to connect HSM to Grafana, you need to complete the following steps:

1. [Preparing Grafana to work with HSM](#preparing-grafana-to-work-with-hsm) (done once on the server)
2. [Preparing HSM to work with Grafana](#preparing-hsm-to-work-with-grafana)
3. [Creating a data source on Grafana](#creating-a-data-source-on-grafana)
4. [Data integration](#data-integration) (creation of dashboards)
...
2. Open **"Connections"** window by Toggle menu
...
This operation is performed once, during the first integration of systems. This completes the preparation for integration.

---

## Preparing HSM to work with Grafana

Working with HSM data is done through **[Access Keys](Access-keys)**.
...
|---|---|
| **Master Key** | Gives access to all server products. Can only be created by admins and is used only for Grafana communication |
| **Product Key** | A specific product key that gives access to one product's data |

![image](https://github.com/SoftFx/Hierarchical-Sensor-Monitoring/assets/43994777/607eca52-f7e2-4df8-844d-fee5e2a9e08a)

In addition to using the key, you must specify on the site exactly what data should be sent. To do this, right-click on the product/node/sensor whose data you want to display on Grafana, and check **"Enable Grafana"**.

![image](https://github.com/SoftFx/Hierarchical-Sensor-Monitoring/assets/43994777/6ec2b79d-d4c9-4f9e-a9cc-2528ca61460f)

Next to the product/node/sensor that was selected, a Grafana icon will appear, which indicates that data for this entity will be sent. (The icon may not be displayed if the visualization of icons is disabled in filters.)
...
1. Click **"Data Sources"** by Toggle menu
...
5. Fill **URL** address, for example: `https://hsm.dev.soft-fx.eu:44333` + `/grafana/jsondatasource`. Full URL: `https://hsm.dev.soft-fx.eu:44333/grafana/jsondatasource`
...
> **Note:** This section is under construction. For supported data types, see [Supported Grafana Datasources](Supported-Grafana-datasources).

---

## See Also

- [Supported Grafana Datasources](Supported-Grafana-datasources) — Which sensor types are supported in Grafana
- [Access Keys](Access-keys) — How to create and manage access keys
